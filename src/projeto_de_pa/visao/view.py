...
class View:
    ...
    def __init__(self, root, controller):
        ...
        self.paddings = {'padx': 5, 'pady': 5} # Espaçamento padrão utilizado nos componentes da interface
    ...
    def criar_menu_figuras(self):
        ...
        option_menu = ttk.OptionMenu(self.frame, tipo_figura_var, 'Linha', *self.traduzir_figura.keys(), command=self.notificar_ferramenta)
    ...
    def criar_menu_borda(self):
        ...
        menu_cor_borda = ttk.OptionMenu(self.frame, cor_borda_var, self.cores_opcoes[0], *self.cores_opcoes, command=self.notificar_cor_borda)
    ...
    def criar_menu_preench(self):
        ...
        menu_cor_preench = ttk.OptionMenu(self.frame, cor_preench_var, self.cores_preench_opcoes[0], *self.cores_preench_opcoes, command=self.notificar_cor_preench)
    ...
    def criar_botao_limpar(self):
        ...
        btn_limpar = ttk.Button(self.frame, text="Limpar Tela", command=self.controller.limpar)

        btn_limpar.grid(column=6, row=0, sticky=W, **self.paddings)

    def criar_canvas(self):
        # Cria o local onde as figuras serão desenhadas
        canvas = Canvas(self.frame, bg='white', width=700, height=600)

        canvas.grid(column=0, row=1, columnspan=7, sticky=W, **self.paddings)
    ...
    def notificar_ferramenta(self, valor_selecionado):
        # Informa ao Controller qual figura foi selecionada
    ...
    def notificar_cor_borda(self, valor_selecionado):
        # Informa ao Controller a nova cor da borda
    ...
    def notificar_cor_preench(self, valor_selecionado):
        # Informa ao Controller a nova cor do preenchimento
        cor = self.traduzir_cor[valor_selecionado]
    ...
    def capturar_movimento_solto(self, event):
        """Repassa ao Controller o movimento do mouse mesmo sem o botão pressionado. Serve para o preview do polígono."""
        self.controller.mover_mouse(event.x, event.y)
    ...
    def _renderizar_figura(self, fig, finalizada): 
        # Desenha uma figura de acordo com sua classe
        ...
        if nome_classe == 'Linha':
            ...
            if len(fig.vertices) >= 3 and finalizada:
                # Converte a linha de vértices para o formato exigido pelo create_polygon()
